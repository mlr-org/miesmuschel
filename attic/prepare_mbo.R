...
#
  ...
  arg("File to save the result under", "file", "f", type = "string", default = "<autogenerated>", name = "filename"),
...
makeEvaluator <- function(config) {
  ...
  evaluate_metaconf <- function(metaconf) {
    ...
    more.args = list(metaconf = metaconf, budgetfactor = configbudgetfactor)
  ...
  fun <- switch(config$algo,
    intermbo = function() {
      designlength <- length(oi$search_space$pars) * 4
...
evalfun <- makeEvaluator(opts, opts$filename)
