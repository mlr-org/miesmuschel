...
ObjectiveIrace = R6Class("ObjectiveIrace", inherit = bbotk::Objective, 
  public = list(
   ...
   private = list(
    .eval = function(xs) {
        objective = cfg$get_objective(task = self$irace_instance, target_variables = "val_balanced_accuracy")
        ...
        if (lambda < 1) return(list(val_balanced_accuracy = 0))

        oi = OptimInstanceSingleCrit$new(objective, search_space,
...
meta_codomain = cfg$get_objective()$codomain$clone()$subset("val_balanced_accuracy")
