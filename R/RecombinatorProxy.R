...
RecombinatorProxy = R6Class("RecombinatorProxy",
  ...
  public = list(
    ...
    prime = function(param_set) {
      ...
      private$.primed_with = primed_with  # keep uncloned copy of configuration parameter value for check in `.recombine()`
      invisible(self)
    }
  ),
  private = list(
    .recombine = function(values) {
      if (!is.null(private$.primed_with) && !identical(self$param_set$get_values()$operation, private$.primed_with)) {
        ...
        stop("RecombinatorProxy$prime() must be called again when the 'operation' configuration parameter changes.")
      }
